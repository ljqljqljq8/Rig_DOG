...
class PhotoProcessor:
    ...
    def process_person_photos(
        ...
        for photo_path in photo_paths:
            ...
            if result["success"]:
                if self.vector_store.add(name, result["embedding"]):
                    loaded += 1
                else:
                    failed.append({"file": result["file"], "error": "failed to store embedding"})
            else:
    ...
    def process_single_photo(
        ...
        strategy = strategy or config.MULTIPLE_FACES_STRATEGY
        person_name = person_name or self._person_name_for_photo(file_path)
        image = self._read_image(file_path)
        if image is None:
            return {
                "success": False,
                "name": person_name,
                "file": self._relative_photo_path(file_path),
                "embedding": None,
    ...
    def _find_person_photos(self, name: str) -> list[Path]:
        return [
            ...
            if self._person_name_for_photo(photo_path) == name
        ]

    def _group_photos_by_person(self, photo_paths: list[Path]) -> dict[str, list[Path]]:
        grouped: dict[str, list[Path]] = {}
        for photo_path in photo_paths:
            grouped.setdefault(self._person_name_for_photo(photo_path), []).append(photo_path)
        return grouped
